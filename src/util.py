...
def compute_score(predictions, labels):
    ...
    #
    precision, recall, F1, mse = score_sequences(predictions, labels)
...
def read_annotation_file(path):
    '''
        ...
        Returns:
            annotation list as dictionary

        Raises:
            Nothing
    '''
    with open(os.path.join(path,"train_anno.json")) as annotation_file:
...
def extract_centroids(pred, bg=0):
    '''
        Returns the centroids of the detected blobs in the masks

        Args:
            mask and background value

        Returns:
            centroids of the detected blobs
